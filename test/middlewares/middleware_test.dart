...
main() {
  ...
  test("load all photo middle ware test", () {
    final repository = MockPhotoRepository();
    final BuiltList<Photo> mockPhotos = Photo.fromJsonList(mockPhotoListJson);
    when(repository.loadPhoto()).thenAnswer((invocation) {
      return Future.value(mockPhotos);
    });

    final store = Store<AppState>(appReducers,
        initialState: AppState.loading(),
        middleware: appMiddleware(repository));

    store.dispatch(PhotoLoadAction());
    verify(repository.loadPhoto()).called(1);
  });
}
